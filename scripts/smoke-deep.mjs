...
if (/^JWT_REFRESH_SECRET=.*JWT_ACCESS_EXPIRES_IN=/m.test(rawEnv)) {
  failNow('Tu .env tiene JWT_REFRESH_SECRET y JWT_ACCESS_EXPIRES_IN pegados en la misma linea. Separalos antes de continuar.');
}
...
async function main() {
  ...
  info(`ALLOW_MUTATIONS=${allowMutations} | EXTERNAL=${external} | PROCESS_OUTBOX=${processOutbox} | BACKUP_DRY_RUN=${backupDryRun}`);
  ...
  assert(Array.isArray(mePatient.data?.roles) && mePatient.data.roles.includes('PATIENT'), 'Paciente tiene rol PATIENT', mePatient);
  const meTherapist = await api('GET', '/me', { token: tokens.therapist, expected: 200 });
  assert(Array.isArray(meTherapist.data?.roles) && meTherapist.data.roles.includes('THERAPIST'), 'Terapeuta tiene rol THERAPIST', meTherapist);
  const meAdmin = await api('GET', '/me', { token: tokens.admin, expected: 200 });
  assert(Array.isArray(meAdmin.data?.roles) && meAdmin.data.roles.includes('ADMIN'), 'Admin tiene rol ADMIN', meAdmin);
  ...
  const newPatientRefresh = required(refreshResponse.data?.refreshToken, 'nuevo refreshToken paciente');
  ...
  await api('GET', '/admin/therapy/approaches?page=1&limit=5', { token: tokens.accountant, expected: 403 });
  ok('Contador no puede administrar catalogo terapeutico');
  await api('GET', '/admin/accounting/accounts?page=1&limit=5', { token: tokens.admin, expected: 403 });
  ...
  assert(Array.isArray(approaches.data) && approaches.data.length >= 1, 'Catalogo publico de enfoques devuelve datos', approaches);
  assert(Array.isArray(products.data) && products.data.length >= 1, 'Catalogo publico de productos devuelve datos', products);
  ...
  const users = await api('GET', '/admin/users?page=1&limit=50', { token: tokens.admin, expected: 200 });
  assert(Array.isArray(users.data) && users.data.length >= 5, 'Admin lista usuarios demo', users);
  const adminApproaches = await api('GET', '/admin/therapy/approaches?page=1&limit=10', { token: tokens.admin, expected: 200 });
  assert(Array.isArray(adminApproaches.data) && adminApproaches.data.length >= 1, 'Admin lista enfoques', adminApproaches);
  await api('POST', '/analytics/ui-events', {
    body: { sessionId: 'smoke-deep-node', eventName: 'SMOKE_DEEP_VISIT', payload: { source: 'node' } },
    expected: 201,
  });
  const analytics = await api('GET', '/admin/analytics/ui-events?page=1&limit=10', { token: tokens.admin, expected: 200 });
  assert(Array.isArray(analytics.data) && analytics.data.length >= 1, 'Admin ve eventos analytics', analytics);
  const audit = await api('GET', '/admin/audit/logs?page=1&limit=10', { token: tokens.admin, expected: 200 });
  assert(Array.isArray(audit.data), 'Admin consulta auditoria', audit);
  const outbox = await api('GET', '/admin/mensajeria/outbox?page=1&limit=10', { token: tokens.admin, expected: 200 });
  ...
  const therapistId = required(users.data.find((u) => u.email === emails.therapist)?.id, 'THERAPIST_ID');
  ...
  assert(Array.isArray(availability.data?.slots), 'Disponibilidad devuelve arreglo de slots', availability);
  ...
  const schedules = await api('GET', '/therapists/me/schedules', { token: tokens.therapist, expected: 200 });
  assert(Array.isArray(schedules.data) && schedules.data.length >= 1, 'Terapeuta lista agenda propia', schedules);
  await api('POST', '/therapists/me/schedules', {
    token: tokens.therapist,
    body: { weekday: 1, startTime: '13:00', endTime: '09:00', timezone: 'America/La_Paz', effectiveFrom: '2026-07-01' },
    expected: 400,
  });
  ok('Agenda rechaza rango invalido');

  step('Contabilidad: lectura y regla de partida doble');
  const groups = await api('GET', '/admin/accounting/account-groups?page=1&limit=20', { token: tokens.accountant, expected: 200 });
  const accounts = await api('GET', '/admin/accounting/accounts?page=1&limit=20', { token: tokens.accountant, expected: 200 });
  assert(Array.isArray(groups.data) && groups.data.length >= 2, 'Contador lista grupos de cuenta', groups);
  assert(Array.isArray(accounts.data) && accounts.data.length >= 2, 'Contador lista cuentas', accounts);
  const debitAccountId = required(accounts.data.find((a) => a.normalBalance === 'DEBIT')?.id, 'cuenta debito demo');
  const creditAccountId = required(accounts.data.find((a) => a.normalBalance === 'CREDIT')?.id, 'cuenta credito demo');
  ...
  if (allowMutations) {
    ...
    assert(String(newPatient.data?.email || '').startsWith('smoke.patient.'), 'Registro paciente crea usuario completo', newPatient);
    ...
    const page = await api('POST', '/admin/cms/pages', {
      token: tokens.admin,
      body: { slug: pageSlug, title: 'Smoke Page', status: 'PUBLISHED', seoMetadata: { description: 'Smoke' } },
      expected: 201,
    });
    const pageId = required(page.data?.id, 'page id');
    await api('POST', `/admin/cms/pages/${pageId}/elements`, {
      token: tokens.admin,
      body: { code: 'hero', type: 'HERO', content: { title: 'Smoke', subtitle: 'OK' }, sortOrder: 1 },
      expected: 201,
    });
...
async function smokeFileUpload({ token }) {
  ...
  if (external && storageProvider !== 'GCS') {
    throw new Error(`Para --external, STORAGE_PROVIDER debe ser GCS. Valor actual: ${storageProvider || 'vacio'}.`);
  }
  ...
  if (upload.__status !== 201) {
    ...
    info(`Upload no bloqueante fallo con HTTP ${upload.__status}. Usa --external para exigir GCS real.`);
    return;
  }
  ...
  if (response.status !== 200) {
    throw new Error(`No se pudo descargar el archivo subido por signed URL. HTTP ${response.status}. Provider=${provider}`);
  }
  ...
  ok(`Imagen PNG subida, descargada y verificada por checksum. Provider=${provider} Size=${downloaded.length}B`);
}

async function smokeSendGrid({ adminToken }) {
  step(`Externo: SendGrid real con correo de prueba a ${testEmail}`);
  const emailProvider = String(process.env.EMAIL_PROVIDER || process.env.MAIL_PROVIDER || '').toUpperCase();
  if (emailProvider !== 'SENDGRID') {
    throw new Error(`Para --external, EMAIL_PROVIDER o MAIL_PROVIDER debe ser SENDGRID. Valor actual: ${emailProvider || 'vacio'}.`);
  }
  const sendgridApiKey = process.env.SENDGRID_API_KEY || '';
  if (!sendgridApiKey) throw new Error('Para --external, debes configurar SENDGRID_API_KEY.');
  if (!sendgridApiKey.startsWith('SG.')) {
    throw new Error("SENDGRID_API_KEY no parece una API key real de SendGrid. Normalmente debe empezar con 'SG.'.");
  }
  ...
  const processResult = await api('POST', `/admin/mensajeria/outbox/${encodeURIComponent(String(testOutboxId))}/process`, {
    token: adminToken,
    expected: 201,
  });
  ...
  if (!resultForThisMessage) {
    throw new Error(`El proceso de mensajeria no devolvio resultado para id=${testOutboxId}. Respuesta=${JSON.stringify(processResult, null, 2)}`);
  }
  if (resultForThisMessage.status !== 'SENT') {
    const reason = resultForThisMessage.lastError || JSON.stringify(resultForThisMessage.responseMetadata || {});
    throw new Error(`SendGrid no envio el correo id=${testOutboxId}. Estado=${resultForThisMessage.status}. Error=${reason}`);
  }

  const outboxAfter = await api('GET', '/admin/mensajeria/outbox?page=1&limit=50', { token: adminToken, expected: 200 });
  const found = Array.isArray(outboxAfter.data) && outboxAfter.data.some((item) => String(item.id) === String(testOutboxId) && item.recipient === testEmail && item.status === 'SENT');
...
async function api(method, path, options = {}) {
  ...
  try {
    ...
    throw new Error(`No se pudo conectar con ${method} ${url}. ¿El backend esta levantado? Error: ${error.message}`);
  }
  ...
  if (!expectedIncludes(expected, response.status)) {
    throw new Error(`HTTP inesperado ${response.status} en ${method} ${path}. Esperado: ${expected}.`);
  }
...
function step(message) { currentStep = message; line(); console.log(`[STEP] ${message}`); }
