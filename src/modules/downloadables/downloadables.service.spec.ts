...
function makeService(opts: { premium?: boolean; entitled?: boolean } = {}) {
  ...
  const versionModel = {
    create: jest.fn(async (v: any) => ({
      ...
      update: jest.fn(async function (this: any, u: any) {
  ...
  const linkModel = {
    findOrCreate: jest.fn(async () => [{ id: 'l1' }, true]),
  ...
  const subscriberModel = {
    findOne: jest
      .fn()
      .mockResolvedValue(
        opts.premium ? { status: 'ACTIVE', subscriptionTier: 'PREMIUM', premiumUntil: null } : null,
      ),
  };
  const hotmart = {
    isConfigured: () => true,
...
describe('DownloadablesService — control de acceso', () => {
  ...
  it('PURCHASE_REQUIRED sin compra + hotmart: checkout', async () => {
    ...
    const d = await service.evaluateAccess(
      resource({
        ...
        hotmartProductId: 'p1',
        hotmartCheckoutUrl: 'https://pay/x',
      }),
      publicUser,
    );
  ...
  it('Admin descarga aunque no publicado', async () => {
    const { service } = makeService();
    const d = await service.evaluateAccess(
      resource({ status: 'DRAFT', visibility: 'PRIVATE' }),
      adminUser,
    );
    expect(d).toMatchObject({ allowed: true, action: 'DIRECT_DOWNLOAD' });
  });
...
describe('DownloadablesService — versionado', () => {
  it('publica solo una version aprobada (inmutable)', async () => {
    const { service, versionModel, store } = makeService();
    const version: any = {
      id: 'v1',
      resourceId: 'r1',
      versionNumber: 2,
  ...
  it('rechaza transicion invalida', async () => {
    ...
    versionModel.findOne.mockResolvedValue({
      id: 'v1',
      resourceId: 'r1',
      status: 'DRAFT',
      update: jest.fn(),
...
describe('DownloadablesService — Hotmart idempotente', () => {
  it('procesa una vez y concede acceso (APPROVED)', async () => {
    ...
    const out = await service.processHotmartNotification({
      eventId: 'evt1',
      productId: 'p1',
      status: 'APPROVED',
      buyerUserId: 'u9',
  ...
  it('segundo evento con mismo id: idempotente (no reprocesa)', async () => {
    ...
    externalEventModel.findOrCreate.mockResolvedValue([
      { processed: true, result: 'ACCESS_GRANTED' },
      false,
    ]);
    const out = await service.processHotmartNotification({
      eventId: 'evt1',
  ...
  it('firma invalida: rechazada', async () => {
    ...
    await expect(
      service.processHotmartNotification({
        eventId: 'evt2',
        productId: 'p1',
        status: 'APPROVED',
      } as any),
    ).rejects.toMatchObject({ response: { code: 'HOTMART_INVALID_SIGNATURE' } });
  });
});
