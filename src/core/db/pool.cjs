...
// SSL controlado por env (default: false para dev)
const sslEnabled =
  String(pick(process.env.PGSSL, process.env.DB_SSL, "false")).toLowerCase() === "true";

const sslRejectUnauthorized =
  String(process.env.PGSSL_REJECT_UNAUTHORIZED || "false").toLowerCase() === "true";

const ssl = sslEnabled ? { rejectUnauthorized: sslRejectUnauthorized } : false;
